...
def hourglass_refinement(netIN_1,netIN_2,netIN_3,netIN_4,netIN_5,training):
	...
	def feature_fusion(f1,f2,f3,f4,f5):
		f11 = tf.concat([f1,f2],3) #1*128*256*256
		f22 = tf.concat([f3,f4],3)
		f22 = tf.concat([f11,f22],3)
		f33 = tf.concat([f22,f5],3)        

		c1 = conv_layer(f33,layer_name('conv'),KER_SZ,NUM_CH[2],bn=bn,training=training) # 1*64*256*256
		cc1= conv_layer(c1,layer_name('conv'),KER_SZ,NUM_CH[2],bn=bn,training=training) # 1*64*256*256
		
			#1*128*256*256
		c2 = conv_layer(cc1,layer_name('conv'),KER_SZ,NUM_CH[1],bn=bn,training=training)
		cc2 = conv_layer(c2,layer_name('conv'),KER_SZ,NUM_CH[1],bn=bn,training=training)
		
		# f33 = tf.concat([cc1,cc2],3) #1*128*256*256
		c3 = conv_layer(cc2,layer_name('conv'),KER_SZ,NUM_CH[0],bn=bn,training=training)
