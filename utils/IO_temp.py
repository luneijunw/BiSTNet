...
## ******************************Write prediction************************************
def write_matrix_depth(Vis_dir,prediction,i):
    image_tensor  = np.asarray(prediction)   
    write_matrix_txt(image_tensor,Vis_dir+str(i)+'.txt')
...
def write_prediction(Vis_dir,prediction,i,idx,Z_r1):
    ...
    mask = Z_r1[num,...,0]>0
    write_matrix_txt(image,Vis_dir+"STEP%d_frame%d_DEPTH.txt" % (i,idx[0]))
...
def write_prediction_normal(Vis_dir,prediction,i,idx,Z_r1):
    ...
    write_matrix_txt(image_unit[...,0],Vis_dir+"STEP%d_frame%d_NORMAL_1.txt" % (i,idx[0]))
    write_matrix_txt(image_unit[...,1],Vis_dir+"STEP%d_frame%d_NORMAL_2.txt" % (i,idx[0]))
    write_matrix_txt(image_unit[...,2],Vis_dir+"STEP%d_frame%d_NORMAL_3.txt" % (i,idx[0]))
...
def save_prediction_png1 (image,imagen,X,Z,Z3,Vis_dir,i,idx,perc):
    ...
    # print((np.array(Image.open(Vis_dir+data_name+"_depth.png")).type())
    d = np.array(np.array(Image.open(Vis_dir+data_name+"_depth.png")),dtype='f')
    d = np.where(Z3[0,...]>0,d[...,0:3],255.0)
    n = np.array(np.array(Image.open(Vis_dir+data_name+"_normal.png")),dtype='f')
    ...
    final_im.save(Vis_dir+"STEP%d_frame%d_results.png" % (i,idx[0]))
    os.remove(Vis_dir+data_name+"_depth.png")
    os.remove(Vis_dir+data_name+"_normal.png")

def save_prediction_png (image,dataname,Z,Vis_dir,itr,perc):
    data_name = dataname
    depth_map = image*Z[0,...,0]
    min_depth = np.amin(depth_map[depth_map>0])
    max_depth = np.amax(depth_map[depth_map>0])*perc 
    depth_map[depth_map < min_depth] = min_depth
    depth_map[depth_map > max_depth] = max_depth
    plt.imsave(Vis_dir+data_name+'_'+itr+".png", depth_map, cmap="hot")   
...
def read_renderpeople(rp_path, frms, IMAGE_HEIGHT,IMAGE_WIDTH):
    ...
    for b in range(Bsize):
        cur_f = int(frms[b])
        name = "%d" %(cur_f)
        print('renderpeople: ',name,b)
        batch_densepose.append(np.array(Image.open(rp_path +'/densepose/'+name+'.png')))
        batch_color.append(np.array(Image.open(rp_path +'/color_WO_bg/'+name+'.png')))
        batch_mask.append(np.array(Image.open(rp_path +'/mask/'+name+'.png')))
        batch_depth.append(np.genfromtxt(rp_path +'/depth/'+name+'.txt',delimiter=","))
        cur_normal = np.array(np.array(Image.open(rp_path +'/normal_png/'+name+'.png')),dtype='f');
    ...
    # print(batch_color.shape,batch_densepose.shape,batch_depth.shape,batch_mask.shape,batch_normal.shape)
...
def read_tiktok(tk_path, frms, IMAGE_HEIGHT,IMAGE_WIDTH):
    ...
    for b in range(Bsize):
        ...
        name = "%d" %(cur_f)
        # print(name)
        batch_densepose.append(np.array(Image.open(tk_path +'/densepose/'+name+'.png')))
        batch_color.append(np.array(Image.open(tk_path +'/color_WO_bg/'+name+'.png')))

        print('tiktok: ',name,b)
        
        batch_mask.append(np.array(Image.open(tk_path +'/mask/'+name+'.png')))
        cur_normal = np.array(np.array(Image.open(tk_path +'/pred_normals_png/'+name+'.png')),dtype='f');
...
def read_correspondences_dp(f,fi,corr_path):
    ...
    name_i = '%d'%(f)
    name_j = '%d'%(fi)
    print('name_i,name_j: ',name_i,name_j)
...
def get_tiktok_patch(tk_path, Bsize, IMAGE_HEIGHT,IMAGE_WIDTH):
    corr_mat = np.genfromtxt(tk_path +'/correspondences/corr_mat_temp2.txt',delimiter=",")
    corr_path = tk_path +'/correspondences/'
    num_of_neighbors = np.shape(corr_mat)[1]-1
    image_nums = corr_mat[0:,0].tolist()
    num_of_ims = len(image_nums)
    frms_nums = np.random.choice(num_of_ims, Bsize).tolist()
    frms= []
    frms_neighbor = []
    ...
    for f in range(len(frms_nums)):
        ...
        neighbor_choice=2
        print("neighbor_choice ",neighbor_choice)
        
        nfrm = corr_mat[row1,neighbor_choice]
        # print(row1,nfrm)
        # print(3/0)
        frms_neighbor = frms_neighbor + [nfrm]
        # if f%5000==0:
        #     print('read corr 5000 +1')
        # print('fra: ',frm)
        # print('nfrm: ',nfrm)
