...
def transform_depth_PCs_dp_based1(C,R,Rt,cen,K,Ki,origin,scaling,d_i,d_j,i_r1_c1_r2_c2,i_limit): #获得预测的前一帧和扭曲的前一帧
    d1 = d_j[0,...,0]
    d2 = d_i[0,...,0]
...
def compute_dp_tr_3d_2d_loss2(d_i,d_j,i_r1_c1_r2_c2,i_limit,C,R,Rt,cen,K,Ki,origin,scaling):
    ...
    PC1p, PC2_1 = transform_depth_PCs_dp_based1(C,R,Rt,cen,K,Ki,origin,scaling,d_i,d_j,i_r1_c1_r2_c2,i_limit)
    ...
    d1 = tf.subtract(PC1p, PC2_1)
    err_vec1 = tf.sqrt(tf.reduce_sum(tf.square(d1),1))
    loss3d1 = tf.reduce_mean(err_vec1)
    x21,_ = reproject(PC1p, K,R,C)
    x1_21,_ = reproject(PC2_1, K,R,C)
    d1 = tf.subtract(x21, x1_21)
    err_vec1 = tf.sqrt(tf.reduce_sum(tf.square(d1),1))
    loss2d1 = tf.reduce_mean(err_vec1)
    return loss3d, loss2d,PC2p, PC1_2,loss3d1, loss2d1,PC1p, PC2_1
