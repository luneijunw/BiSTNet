...
def hourglass_refinement(netIN_1,netIN_2,netIN_3,netIN_4,netIN_5,training):
	...
	def attention(feature_forward,feature_backward):
		...
		f = tf.concat([feature_forward1,feature_backward1],3) #(1, 16, 16, 512)
	...
	# 	pred_depth=conv_layer(x1,layer_name('conv'),1,1,bn=False,relu=False)
		# return h, pred_depth
	def GRU_1(f1,f2):
		f1=tf.shape(f1,[1,16*16,1024]);f2=tf.shape(f2,[4,16*16,1024])
		a1=Bidirectional(GRU(1024,return_sequences=True, dropout=0.25, time_major=False))
		y1=a1(f1)
		y2=tf.nn.relu(y1[-1])
		fc1=tf.keras.layers.Dense(units=1024,input_shape=(1024,))
		forward=fc1(y2)
		a3=Bidirectional(GRU(1024,return_sequences=True, dropout=0.25, time_major=False))
		y3=a3(f2)
		y4=tf.nn.relu(y3[-1])
		fc2=tf.keras.layers.Dense(units=1024,input_shape=(1024,))
		backward=fc2(y4)
	...
	def GRU_bid():
		...
		# f_expand_dim1 = tf.zeros([1,5,16, 16, 1024])
		# f_expand_dim1[:,0,:,:,:]=f_d1;f_expand_dim1[:,1,:,:,:]=f_d2;f_expand_dim1[:,2,:,:,:]=f_d3;f_expand_dim1[:,3,:,:,:]=f_d4;f_expand_dim1[:,4,:,:,:]=f_d5
		f_d11=tf.reshape(f_d1,[1,16*16,256]);f_d22=tf.reshape(f_d2,[1,16*16,256]);f_d33=tf.reshape(f_d3,[1,16*16,256])
		f_d44=tf.reshape(f_d4,[1,16*16,256]);f_d55=tf.reshape(f_d5,[1,16*16,256])
		print('f_d11',f_d11.shape)
		f1= tf.concat([f_d1, f_d2], 3);f2= tf.concat([f1, f_d3], 3);f3= tf.concat([f2, f_d4], 3);f4= tf.concat([f3, f_d5], 0)
		print('f1',f1.shape)
		f1f,f1b=GRU_1(f4);f2f,f2b=GRU_2(f4);f3f,f3b=GRU_3(f4);f4f,f4b=GRU_4(f4);f5f,f5b=GRU_5(f4);
		...
		# input_cat1 = tf.concat([forward, backward], 0)
		# attention(input_cat1[0],input_cat1[1])
	...
	def Bidi_CLSTM():#f_d.shape=[10,256,256,64]
		...
		f1= tf.concat([f_d1, f_d2], 0);f2= tf.concat([f1, f_d3], 0);f3= tf.concat([f2, f_d4], 0);f4= tf.concat([f3, f_d5], 0)
	...
	out1,out2,out3,out4,out5=GRU_bid()
	return out1,out2,out3,out4,out5
